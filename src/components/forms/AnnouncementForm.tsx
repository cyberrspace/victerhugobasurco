...
export default function AnnouncementForm() {
  ...
  if (state === 'sent') {
    return (
      <div className="border border-ember/40 bg-night/60 p-8" role="status">
        <span className="eyebrow">You are on the list</span>
        <p className="mt-4 font-display text-[1.4rem] leading-snug text-bone">
          You will hear from Victer when there is a book or a date to announce.
        </p>
        <p className="mt-3 text-[0.92rem] text-ash">Nothing else. No monthly letter.</p>
      </div>
    );
  }

  return (
    <form onSubmit={onSubmit} noValidate className="border border-bone/15 bg-night/70 p-7 backdrop-blur-sm sm:p-9">
      <h3 className="font-display text-[1.6rem] leading-tight text-bone">Announcements only</h3>
      ...
      <button type="submit" className="btn-ember mt-7 w-full" disabled={state === 'sending'}>
        <span>{state === 'sending' ? 'Adding you…' : 'Add me to the list'}</span>
      </button>

      <p className="mt-4 text-[0.78rem] leading-relaxed text-ash/80">
        Your address is used for announcements from this site only, and never shared.
      </p>

      <p aria-live="polite" className="mt-3 min-h-[1.25rem] text-[0.85rem] text-ember">
        {state === 'error' ? message : ''}
      </p>
    </form>
  );
