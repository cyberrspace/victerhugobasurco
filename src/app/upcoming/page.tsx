...
export const metadata: Metadata = {
  title: 'Upcoming',
  description: 'Forthcoming books and appearance dates for Victer Hugo Basurco.',
};
        ...
        <div className="shell">
          ...
          {upcoming.length === 0 && (
            <Reveal variant="up" delay={80}>
              <p className="mt-8 max-w-prose text-[1.02rem] leading-relaxed text-ash">
                No dates are scheduled at the moment. New ones are posted here first — join the
                announcement list on the home page to hear about them.
              </p>
            </Reveal>
          )}
            ...
            {[...upcoming, ...past].map((e, i) => {
              ...
              return (
                <Reveal key={e.id} as="li" variant="up" delay={i * 80}>
                  <div
                    className={`group grid items-baseline gap-4 border-t border-bone/10 py-7 md:grid-cols-[10rem_1fr_auto] ${
                      isPast ? 'opacity-55' : ''
                    }`}
                  >
                    <time
                    ...
                    <div>
                      <h3 className="font-display text-[1.3rem] text-bone">{e.venue ?? e.title}</h3>
                      <p className="mt-1 text-[0.95rem] text-ash">
                        {e.city}
                        {e.time ? ` · ${e.time}` : ''}
                      </p>
                      <p className="mt-3 max-w-prose text-[0.95rem] leading-relaxed text-ash">{e.excerpt}</p>
                    </div>
                    <span className="font-condensed text-[0.62rem] uppercase tracking-[0.24em] text-ash">
